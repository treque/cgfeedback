...
void calculerPhysique( )
{
    ...
    if ( Etat::enmouvement )
    {
		...
		// on indique ici que les valeurs demeurent dans le GPU et qu'on les stocke dans un VBO cree a l'avance
		glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 0, vbo[1]);

		glBindVertexArray(vao[1]);
		glBindBuffer(GL_ARRAY_BUFFER, vbo[0]);

		// locations of needed vars for calculations in shader
        ...
        // « dessiner »
		// commencer la transf de retroaction pr commencer capture
...
void chargerNuanceurs()
{
    ...
    std::cout << "Charger le nuanceur de sommets pour la rétroaction" << std::endl;
    {
        ...
        // À MODIFIER (partie 1)
		// voici les informations que nous voulons recevoir (tableau d'attribs)
...
void definirProjection( int OeilMult, int w, int h ) // 0: mono, -1: oeil gauche, +1: oeil droit
{
	...
	GLdouble proportionProfondeur = VueStereo::zavant / VueStereo::zecran;
